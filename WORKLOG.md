...
**작업 목적**: 이미지 검색 유입 확대 + 네이버 키워드 최적화

**`lib/data/calculators.ts` 수정**:
- `Calculator` 인터페이스에 `icon: string` 필드 추가
- 21개 계산기 전체에 이모지 아이콘 할당 (🏠 💰 📈 🏦 🔑 📋 🏢 🧾 📐 💱 ⚖️ 🔥 🥗 📅 📆 🎂 💼 🎓 ⚡ 🔢 🚗)
- 전체 `description` 필드 네이버 검색패턴 언어로 재작성 (예: "연봉별 월 실수령액, 4대보험·소득세 공제 내역 자동 계산")
- 전체 `seo.description` 필드 구체적 금액/시나리오 포함 (예: "연봉 3000만원·5000만원·1억원별 월 실수령액")
- `keywords` 배열에 네이버 검색 변형어 추가 (예: "연봉계산기", "월급계산기", "세후연봉")

**`components/common/CalculatorCard.tsx` 신규 생성**:
- 이모지 아이콘 + 계산기명 + 설명 3단 구조
- hover 시 border + shadow 전환 + name 색상 변경
- 5개 카테고리 페이지 공용 컴포넌트로 재사용

**카테고리 페이지 5종 업데이트** (`finance`, `health`, `date`, `life`, `convert`):
- Link 직접 렌더링 → `<CalculatorCard>` 컴포넌트 교체
- 히어로 섹션에 카테고리별 인라인 SVG 일러스트 추가 (md+ 화면에서만 노출)
  - 금융: 상승 바차트 + 꺾은선 그래프
  - 건강: 심박수 파형이 있는 물방울
  - 날짜: D 마커가 있는 달력
  - 생활: 사방으로 빛이 퍼지는 전구
  - 변환: 양방향 화살표 + 평/m² · KRW/USD 레이블

**빌드 검증**: TypeScript 오류 없음, 전체 static prerender 성공
