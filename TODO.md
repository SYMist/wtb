...
## 금통위 인상 즉시 반영 — 기준금리 2.75% → 3.00% (2026-08-27 · Avatar 발주)

> 근거: `wiki/web/performance.md` "🔧 2026-08-27 금통위 인상 즉시 반영 (③수리, 게이트 면제) — 실행 스펙". 유형 = **③수리**(①신설 카운트 아님).
> 급소: ECOS 월시계열 발행 지연 + cron이 매월 1일 1회 ⇒ 손대지 않으면 **최대 5주간 옛 값**. 승리 표면 `/data/rates/base`의 유일 해자("자동 갱신으로 지금 맞는 값")가 정면으로 무너지는 구간.
> 가드 확인됨(8/01 사고 후 `scripts/lib/series.ts`): ⓐ `mergeSeries`가 수동 월 보존 ⓑ ECOS 발행 시 공식값이 이김 ⓒ 역행 가드는 머지 후 latest 기준이라 오발 없음.

- [x] **① 데이터 반영** — `base-rate-series.json`: `2026-08 / 3` 추가(320점) · `latest` 동일 · `stats.average` **2.84 유지**(재계산 결과 불변) · `max/min` 불변(5.25 / 0.50 — 9/08 스니펫 판정 캐비엇과 충돌 없음) · `updatedAt: 2026-08-27` · **`checkedAt: 2026-08-05` 그대로**. 커밋 `d5c3f0f` (2026-08-27)
- [x] **② 파생 확인** — `fetch-interest-rates.ts` 실행 로그 `baseRate from base-rate-series.json latest: 2026-08 3%` → `interest-rates.json` `baseRate: 3` · `updatedAt: 2026-08-27` (2026-08-27)
- [x] **③ 테스트** — `series.test.ts` **11/11 passed** (보존 ⓐ·정정 ⓑ·역행 throw ⓒ 전부 green) (2026-08-27)
- [x] **④ 완료기준 ⓐ** — 라이브 `/data/rates/base` **HTTP 200** · 월별표 `2026-08 | 3.00% | +0.25%p` · 신선도 배지 `데이터 기준 2026년 8월` · Hero 「현재」 카드 `3%` (2026-08-27)
- [x] **④ 완료기준 ⓑ** — 라이브 `/finance/rent-conversion` **HTTP 200** · `법정 전환율: 5.00% (기준금리 3% + 2%)` 2곳 · **`4.75%` 잔존 0건**. 회귀값 상한 5.00% 통과 (2026-08-27)
- [x] **④ 완료기준 ⓒ** — `git status -sb` = `## main...origin/main` (ahead 0). push `4dd28c6..d5c3f0f`, Cloudflare 자동 빌드 ~2분 후 라이브 반영 확인 (2026-08-27)
- [ ] **⑤ 판정선 (9/01)** — 자동 run 후 `2026-08` 행 **생존**(`preserved ≥ 1` 로그). 사라지면 가드 실패 ⇒ 8/01 사고 재발로 격상
- [ ] 🔎 **곁다리 발견 — 정수 금리 표기 불일치 (스펙 예상과 어긋난 1건, 미조치)**: 발주 스펙은 `3.00%` 표시를 예상했으나 **`{latest.rate}%` 원시 렌더 자리는 `3%`로 나온다**(기준금리가 사상 처음 정수값이 됐다). 값은 정확하고 상한 회귀도 통과하므로 **완료기준 실질 충족**이나, 문자열이 갈린다.
  - `3.00%` (toFixed(2) 경유): 월별 추이표, 연도별 평균표
  - `3%` (원시): base Hero 「현재」 카드 · 본문 프로즈 3곳 · **`<meta og:title>`·`twitter:title` = `한국은행 기준금리 3% (2026-08)`** · rent-conversion 문구 2곳
  - ⚠️ **조치 보류 사유** — 고칠 자리가 og/twitter title이라 **9/08 스니펫 판정 중인 문자열을 건드리게 된다.** 발주 스펙 밖(스펙은 "데이터 1곳"이 전부라고 명시) ⇒ 볼트 판단 요청. 고친다면 `{latest.rate}` → `{latest.rate.toFixed(2)}` 3파일 6곳
