...
- [x] 네이버 블로그 퇴직금 포스팅 실제 발행
- [x] 네이버 블로그 전세 vs 월세 포스팅 실제 발행
...
- [ ] **청약 가점 계산기** ← **다음 코드 작업** — 무주택 기간·부양가족수·청약통장 가입 기간 입력 → 청약 가점 자동 계산. 검색량 높고 경쟁 사이트 UX가 취약해 차별화 가능. GA4 데이터 없는 지금은 검색량 기반으로 우선순위 판단.
