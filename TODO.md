...
- [x] **양도세 페이지 title/h1/meta 키워드 보강** — 짧은 형 "양도세" + "12억"을 title·h1·레지스트리에 추가(검색자는 구어체 "양도세"·"12억"로 검색하는데 정식명만 있던 갭). title: `양도소득세(양도세) 계산기 - 1세대1주택 12억 초과 고가주택 안분`, keywords에 고가주택·12억 초과 양도세 등 (2026-06-02) ⏳ *배포 대기(wrangler 재로그인 필요)*
- [x] **양도세 페이지 12억 초과 시나리오 프리셋 버튼** — "12억 이하(비과세)/15억 1주택/20억 1주택" 빠른 설정으로 검색의도 즉시 충족 (2026-06-02) ⏳ *배포 대기*
- [x] **네이버 블로그 양도세 후속 글 발행 완료** — 비교형(12억/15억/20억 양도가별 세금 비교)으로 S1과 차별화(유사문서 회피). 프리셋 딥링크 CTA 포함 (`marketing/naver-post-capital-gains-by-price.md`, 2026-06-02 발행)
- [ ] (선택, 볼륨 확인 후) 인접 갭 메우기: **퇴직소득세** 계산(현 퇴직금과 구분) — `퇴직소득세 예상금액` 검색유입 존재. ⚠️ 프로그래매틱 양산은 지양(noindex 교훈)
