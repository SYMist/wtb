/**
 * 원/일본엔(100엔) 매매기준율 월평균 / ECOS 731Y004 item 0000002(원/일본엔) × 0000100(평균자료)
 *
 * 실행: npx tsx --env-file=.env.local scripts/fetch-jpykrw-rate-series.ts
...
const OUTPUT_PATH = join(
  process.cwd(),
  "lib/data/jpykrw-rate-series.json",
);

interface Point {
  date: string; // "YYYY-MM"
  rate: number; // 원 (KRW per 100 JPY, 월평균)
}
...
async function fetchSeries(): Promise<Point[]> {
  ...
  const url = `https://ecos.bok.or.kr/api/StatisticSearch/${API_KEY}/json/kr/1/2000/731Y004/M/${start}/${end}/0000002/0000100`;
...
async function main() {
  ...
  console.log(
    `Saved ${series.length} points (${series[0].date} ~ ${result.latest.date}, latest=${result.latest.rate}원/100엔, dataChanged=${dataChanged}) to ${OUTPUT_PATH}`,
  );
}
