...
import { activityLevels, type ActivityLevel } from "@/lib/calculators/calorie";
...
export default async function CalorieCalculatorPage({ searchParams }: PageProps) {
  const params = await searchParams;
  const initialGender: Gender = str(params.gender) === "female" ? "female" : "male";
  const activityRaw = str(params.activity);
  const initialActivity: ActivityLevel = activityLevels.some(
    (a) => a.value === activityRaw
  )
