...
export default function LoanCalculatorClient({
  ...
  initialGrace,
  initialIncome,
}: LoanCalculatorClientProps) {
  const [price, setPrice] = useState(initialPrice);
  const [loanAmountInput, setLoanAmountInput] = useState(initialLoan);
  const [annualRate, setAnnualRate] = useState(initialRate);
  const [termYears, setTermYears] = useState(initialTerm);
  const [gracePeriodYears, setGracePeriodYears] = useState(initialGrace);
