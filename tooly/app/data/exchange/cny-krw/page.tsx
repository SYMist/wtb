...
const data = cnykrwData as SeriesData;

const PAGE_URL = "https://tooly.deluxo.co.kr/data/exchange/cny-krw";
...
export const metadata: Metadata = {
  title: "원/위안 환율 추이 (월평균 매매기준율)",
  description: `${data.latest.date} 기준 원/중국위안 월평균 환율은 ${fmtWon(data.latest.rate)}. 2006년 이후 월별 시계열과 주요 흐름을 확인하세요.`,
  alternates: { canonical: PAGE_URL },
  openGraph: {
    title: `원/위안 환율 ${fmtWon(data.latest.rate)} (${data.latest.date})`,
    description: "한국은행 ECOS 기반 원/중국위안 월평균 매매기준율 시계열.",
    url: PAGE_URL,
    type: "article",
  },
};
...
export default function CnyKrwPage() {
  ...
  const datasetSchema = {
    ...
    "@type": "Dataset",
    name: "원/중국위안 환율(매매기준율) 월평균",
    description:
      "한국은행 ECOS 731Y004 (원/중국위안 · 평균자료) — 대원화 환율 월평균 시계열.",
    url: PAGE_URL,
    ...
    variableMeasured: "원/위안 환율 (KRW per CNY)",
    inLanguage: "ko",
  };

  const faq = [
    {
      q: "위안 환율이 한국 경제에 중요한 이유는?",
      a: "중국은 한국 최대 교역국입니다. 위안 가치 변동이 반도체·화학·철강 등 주요 수출 품목에 직접적인 영향을 미칩니다.",
    },
    {
      q: "데이터 출처는?",
      a: "한국은행 ECOS 통계 731Y004 (주요국 통화의 대원화환율) 중 원/중국위안 평균자료 항목 (0000019)입니다.",
    },
    {
      q: "위안화 절하(위안 약세)가 한국에 미치는 영향은?",
      a: "중국산 수입품 가격이 낮아집니다. 반면 한국 수출기업이 중국 업체와 경쟁하는 시장에서 가격 경쟁력이 악화됩니다.",
    },
    {
      q: "원/달러 환율과 어떤 관계?",
      a: "위안/달러 환율 변동이 원/달러에도 동반 영향을 미치는 경우가 많습니다. 달러 강세 시 위안과 원화가 동시에 약세를 보이는 경향이 있습니다.",
    },
    {
      q: "실시간과 왜 다른가요?",
  ...
  const breadcrumbSchema = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 4, name: "원/위안", item: PAGE_URL },
    ],
  };
    ...
    <>
      ...
      <main className="mx-auto w-full max-w-4xl flex-1 px-4 py-8">
        <nav className="mb-4 text-xs text-text-secondary">
          ...
          <span className="mx-1">/</span>
          <span className="text-text-primary">원/위안</span>
        </nav>
        ...
        <section className="mb-8">
          ...
          <p className="mb-6 text-sm text-text-secondary">
            한국은행 ECOS 기준 원/중국위안 매매기준율의 월평균 시계열. 최대 교역국 통화의 흐름을 파악하는 핵심 지표.
          </p>
        ...
        <section className="mb-8 space-y-4 text-sm leading-relaxed text-text-secondary">
          ...
          <p>
            {formatYM(latest.date)} 원/위안 월평균 환율은{" "}
            ...
            {yoy && (
              <>
                ...
                {yoy.percent > 0 ? "원화 대비 위안화 강세" : "원화 대비 위안화 약세"}로 움직였습니다.
              </>
            )}
          </p>
          <p>
            ...
            입니다. 중국은 한국의 최대 교역국으로, 위안화 가치 변동은 한국의 대중 수출입에 직접적인 영향을 미칩니다.
          </p>
          <p>
            위안화는 중국 인민은행(PBOC)이 매일 기준환율(고시환율)을 고시하는 관리변동환율제 통화입니다. 위안/달러 환율 변동이 원/위안 환율에도 연동되므로, 달러 강세 국면에서 위안과 원화가 동시에 약세를 보이는 경향이 있습니다.
          </p>
        </section>
        ...
        <section className="mb-8 rounded-lg border border-primary/30 bg-primary/5 p-5">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <Link
              href="/data/exchange/compare?cur=cny"
              className="rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-text-primary transition-colors hover:bg-surface"
            >
              시점 비교·환산
            </Link>
        ...
        <section className="mb-8 rounded-lg border border-border bg-surface p-5 text-xs text-text-secondary">
          ...
          <ul className="list-inside list-disc space-y-1">
            <li>
              ...
              항목 0000019 (원/중국위안) × 0000100 (평균자료)
            </li>
